...
//
// Real transforms of even length run as one half-length complex FFT
// plus a split pass, with every twiddle precomputed at plan time: at
// n = 960 a forward + inverse pair costs ~10 µs on an M4 Max, about
// a tenth of the per-frame network cost.
//
// The inverse ignores any imaginary part on the DC and Nyquist bins
// (the deep filter's complex coefficients can leave one there) — the
// same convention as a full complex inverse that keeps only the real
// output, and as numpy's irfft.
