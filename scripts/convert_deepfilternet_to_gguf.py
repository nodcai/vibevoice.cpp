...
The result is what scripts/merge_dfn_gguf.py (or convert_vibevoice_to_gguf.py
--dfn) embeds into a VibeVoice gguf as the TTS post-filter.
...
the tensor names `src/dfn_fast.cpp` expects against the actual
export.
"""
...
def convert(input_dir: Path, output: Path, verbose: int) -> None:
    ...
    if total == 0:
        print("  warning: no trained tensors found — the resulting GGUF will load via",
              "          src/dfn.cpp will refuse to load the filter.", file=sys.stderr)
